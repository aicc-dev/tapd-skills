---
name: aicc-tapd:story
description: TAPD 命名空间需求技能，用于读取、更新 TAPD 需求，以及通过 TestX design 接口读取需求。依赖 aicc-tapd:base 提供 OAuth 和用户态 API token；需要把需求关联到 TestX 时可继续使用 aicc-tapd:testx。
---

# TAPD Story 操作

使用这个 skill 处理 TAPD 需求读取、更新，以及 TestX design 侧的需求读取。执行前必须先确认相邻目录存在 `tapd-base`，并按 `aicc-tapd:base` 完成配置和 token 检查。

## 串联方式

- 需求读取/更新：`aicc-tapd:base` -> `aicc-tapd:story`
- 读取需求后创建或关联 TestX 用例：`aicc-tapd:base` -> `aicc-tapd:story` -> `aicc-tapd:testx`
...
```bash
python3 tapd-story/scripts/tapd_story.py \
  story-get \
